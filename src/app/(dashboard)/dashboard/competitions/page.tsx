...
import { db } from '@/db';
import { competitions, clubMembers, clubs as clubsTable, organizations as organizationsTable, competitionRegistrations } from '@/db/schema';
...
export default async function CompetitionsListPage() {
  ...
  let myCompetitionIds: number[] = [];

  if (role === 'player' || role === 'club_president') {
    // Get all clubs where user is a member OR president
    const userMemberClubs = await db.query.clubMembers.findMany({
      where: eq(clubMembers.userId, userId),
    });
    const userPresidentClubs = await db.query.clubs.findMany({
      where: eq(clubsTable.presidentId, userId),
    });

    const clubIds = Array.from(new Set([
      ...userMemberClubs.map(m => m.clubId),
      ...userPresidentClubs.map(c => c.id)
    ]));

    if (clubIds.length > 0) {
      const registrations = await db.query.competitionRegistrations.findMany({
        where: inArray(competitionRegistrations.clubId, clubIds),
      });
      myCompetitionIds = registrations.map(r => r.competitionId);
    }
  } else if (role === 'org_president') {
    const userOrgs = await db.query.organizations.findMany({
      where: eq(organizationsTable.presidentId, userId),
    });
    const orgIds = userOrgs.map(o => o.id);
    
    if (orgIds.length > 0) {
      myCompetitionIds = allCompetitions
        .filter(c => c.organizationId && orgIds.includes(c.organizationId))
        .map(c => c.id);
    }
  }

  const myCompetitions = allCompetitions.filter(c => myCompetitionIds.includes(c.id));
  const otherCompetitions = allCompetitions.filter(c => !myCompetitionIds.includes(c.id));

  const renderGrid = (list: typeof allCompetitions, title: string, subtitle?: string) => (
    <div className="space-y-6">
      <div className="flex flex-col">
        <h3 className="text-xl font-black text-ice tracking-tight uppercase italic flex items-center gap-2">
          {title === 'Inscritos' ? '🔥 ' : '🌍 '}{title}
        </h3>
        {subtitle && <p className="text-ice/30 text-xs font-bold uppercase tracking-widest">{subtitle}</p>}
      </div>
    ...
    <div className="space-y-12">
      ...
      {allCompetitions.length === 0 ? (
        ...
        <div className="space-y-16">
          {myCompetitions.length > 0 && renderGrid(
            myCompetitions, 
            role === 'org_president' ? 'Minhas Competições' : 'Inscritos',
            role === 'org_president' ? 'Gerenciadas por você' : 'Seu clube está na disputa'
          )}

          {otherCompetitions.length > 0 && renderGrid(
            otherCompetitions, 
            'Explorar Torneios',
