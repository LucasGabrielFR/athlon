...
export default async function CompetitionsListPage() {
  ...
  let otherCompetitions: any[] = [];

  try {
    const data = await fetchApi('/competitions/dashboard');
    allCompetitions = data?.all || [];
    myCompetitions = data?.mine || [];
    otherCompetitions = data?.others || [];
  } catch(e) {}
