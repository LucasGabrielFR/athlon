...
export default async function MatchManagementPage({ 
  ...
  let data: any = null;
  try {
    data = await fetchApi(`/competitions/${compId}/matches/${matchId}`);
  } catch(e) {}
          ...
          <div className="lg:col-span-1 space-y-6">
             ...
             {match.status !== 'finished' && (
                ...
                homeTeam={{
                   ...
                   roster: match.homeRegistration.roster.map((r: any) => ({ userId: r.userId, name: r.user.name }))
                }}
                awayTeam={{
                   id: match.awayRegistrationId!,
                   name: match.awayRegistration.club.name,
                   roster: match.awayRegistration.roster.map((r: any) => ({ userId: r.userId, name: r.user.name }))
                }}
                statTypes={modStats}
                compId={compId}
