...
export default async function CompetitionDetailPage({ 
  ...
  let data: any = null;
  try {
    data = await fetchApi(`/competitions/${compId}/dashboard-details`);
  } catch(e) {}

  if (!data || !data.competition) notFound();

  const comp = data.competition;
  const user = data.user;
  const eligibleClubs = data.eligibleClubs || [];
  const myRegistrations = data.myRegistrations || [];
  const allRegistrations = data.allRegistrations || [];
  const clubIds = data.clubIds || [];
  
  const allStandings = data.allStandings || [];
