...
export default async function ClubsPage({
  ...
  try {
    ...
    const [membershipsRes, invitesRes, modalitiesRes, clubsRes] = await Promise.all([
      fetchApi('/users/me/memberships'),
      fetchApi('/users/me/invitations/pending'),
      fetchApi('/modalities'),
      fetchApi(`/clubs?${queryParams.toString()}`)
    ]);

    myMemberships = Array.isArray(membershipsRes) ? membershipsRes : [];
    pendingInvites = Array.isArray(invitesRes) ? invitesRes : [];
    allModalities = Array.isArray(modalitiesRes) ? modalitiesRes : [];
    allClubs = clubsRes?.data || [];
    totalItems = clubsRes?.total || 0;
  } catch (error) {
    // Silent fail
  }
