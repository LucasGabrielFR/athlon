...
export default async function NewClubPage({
  ...
  let activeModalities: any[] = [];
  try {
    const [modalities, memberships] = await Promise.all([
      fetchApi('/modalities'),
      fetchApi('/users/me/memberships')
    ]);

    const myModalityIds = Array.isArray(memberships) ? memberships.map((m: any) => m.modalityId) : [];
    
    if (Array.isArray(modalities)) {
      activeModalities = modalities.filter((m: any) => m.isActive && m.isTeamBased && !myModalityIds.includes(m.id));
    }
  } catch(e) {}
