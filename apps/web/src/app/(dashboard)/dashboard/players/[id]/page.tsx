...
export default async function PlayerProfilePage({
  ...
  try {
    playerData = await fetchApi(`/players/${targetId}/details`);
  } catch(e) {}

  if (!playerData || playerData?.player?.role !== 'player') notFound();
  ...
  let myClubs: any[] = [];
  try {
    myClubs = await fetchApi('/users/me/presided-clubs');
  } catch(e) {}

  const isPresident = Array.isArray(myClubs) && myClubs.length > 0;
